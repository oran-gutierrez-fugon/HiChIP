...
for(f in files) {
  ...
  #Conserves original name for bed file rename (old.file)
  old.file <- file
  
  #Finds part of name and corrects for variable "file" (NOT WORKING)
  ifelse(grep(".interactions_FitHiC_Q0.10_MergeNearContacts.bed",file),sub(".interactions_FitHiC_Q0.10_MergeNearContacts.bed","-10.bed",file),0)
  #makes sure only changes name when one of these conditions is met (NOT WORKING)
  ifelse(grep(".interactions_FitHiC_Q0.01_MergeNearContacts.bed",file),"-01.bed", 0)
  ifelse(grep(".interactions_FitHiC_Q0.05_MergeNearContacts.bed",file),"-05.bed", 0)
  
  #Tries to rename actual bed files (NOT WORKING)
  file.rename(file.path({XX},old.file), file.path({XX}, file))
  
  #old file rename attempts 
  ...
  #Takes out punctuation for graph naming
